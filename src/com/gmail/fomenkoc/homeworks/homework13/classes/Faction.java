...
public class Faction {
	...
	return "Faction [factionName=" + factionName + ", deputies="
		+ deputies + "]";
    }
	...
	System.out.println("Is he a briber?");
	String briber = scanner.next();
	boolean bribeTaker = false;
	if (briber.equalsIgnoreCase("yes")
		|| briber.equalsIgnoreCase("так")
		|| briber.equalsIgnoreCase("true")) {
	    bribeTaker = true;
	}

	this.deputies.add(new Deputy(weight, height, firstNmae,
		lastNmae, bribeTaker));
	...
	while (iterator.hasNext()) {
	    ...
	    if (nextDeputy.getFirstName().equalsIgnoreCase(firstNmae)
		    && nextDeputy.getLastName()
			    .equalsIgnoreCase(lastNmae)) {
		System.out.println("Deputy "
			+ nextDeputy.getFullName() + " removed from "
			+ this.getFactionName());
		iterator.remove();
	    }
	}
